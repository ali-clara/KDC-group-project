...
[t_heating,x_heating] = ode45(@(t,x) odefcn_heating(t,x,m_total,b_origami,k_origami,heating_force,heating_t,pressure_term,tca_num), tspan, x0);
...
function dxdt = odefcn_heating(t, x, m, b, k, input_force, input_t, pressure_force, tca_num)
    f_t = tca_num*interp1(input_t, input_force, t, 'linear', 'extrap');
    f_eq = f_t*(1 - pressure_force);
    dxdt = zeros(2,1);
    dxdt(1) = (-b/m)*x(1) + (-1/m)*x(2) + (1/m)*f_eq;
    dxdt(2) = k*x(1);
end
