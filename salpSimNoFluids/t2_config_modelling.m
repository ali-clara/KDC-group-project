...
k_spring = 300*2; % spring constant of the extension spring (N/m)
k_eq = k_spring - k_origami; % the origami is working against the spring

k_tca = 6*25.4; % (N/m) - spring constant of TCAs between 0-4mm, becomes more nonlinear after that
b_tca = 100; % In water, cooling time constant = 1 (tau = B/K)

% Set up the state matrices
A = [-b_origami/m_cap, -1/m_cap; k_eq, 0];
B = [1/m_cap; 0];
C = [0, 1/k_eq; 1, 0];
D = [0; 0];
...
[t_heating,x_heating] = ode45(@(t,x) odefcn_heating(t,x,m_cap,b_origami,k_eq,heating_force,heating_t), tspan, x0);

% use the state variables (x) to find the output variables (y)
sz = size(x_heating); 
y_heating = zeros(sz); 
y_heating(:,1) = (1/k_eq)*x_heating(:,2); % displacement of the mass
...
% find the state variables (x)
[t_cooling, x_cooling] = ode45(@(t,x) odefcn_cooling(t, x, m_cap, b_origami, k_eq, b_tca, k_tca), tspan, x0);

% use the state variables (x) to find the output variables (y)
sz = size(x_cooling); 
y_cooling = zeros(sz); 
y_cooling(:,1) = (1/k_eq)*x_cooling(:,2); % displacement of the mass
...
function dxdt = odefcn_heating(t, x, m, b, k, input_force, input_t)
    ...
    dxdt = zeros(2,1);
    dxdt(1) = (-b/m)*x(1) + (-1/m)*x(2) + (1/m)*6*f_t;
    dxdt(2) = k*x(1);
end

function dxdt = odefcn_cooling(t, x, m, b_origami, k_eq, b_tca, k_tca)
    dxdt = zeros(3,1);
    dxdt(1) = (-b_origami/m)*x(1) + (-1/m)*x(2) + (-1/m)*x(3);
    dxdt(2) = k_eq*x(1);
    dxdt(3) = k_tca*x(1) + (-k_tca/b_tca)*x(3);
end
